...
/**
 * Strips common IIITK suffixes from a display name.
 * e.g. "EBIN THOMAS -IIITK" → "EBIN THOMAS"
 *       "EBIN THOMAS -IIItK" → "EBIN THOMAS"
 */
 ...
 *
 * Pattern: <name><YY><branch><NN>@iiitkottayam.ac.in
 * Roll No format: 20<YY>BCS<0...NN> (zero-padded to 4 digits)
 *
 * Falls back to the raw local-part if the pattern doesn't match.
 */
function rollFromEmail(email = '') {
    const local = email.split('@')[0]; // "ebinthomas24bcs99"

    // Match: any letters (name), 2-digit year, 3-letter branch, 1-4 digit number
    const match = local.match(/[a-z]+(\d{2})([a-z]{3})(\d{1,4})$/i);
    if (!match) return local; // fallback

    const [, yy, branch, num] = match;
    const year = `20${yy}`;
    const paddedNum = num.padStart(4, '0');
    return `${year}${branch.toUpperCase()}${paddedNum}`; // "2024BCS0099"
}

// ==========================================
// 1. ADMIN: CREATE FORM (Auto-creates Google Sheet)
// ==========================================
router.post('/forms', authenticateToken, requireAdmin, async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({
            message: "Refund form and Google Sheet created successfully!",
            form: result.rows[0]
        });
...
// ==========================================
// 2. ADMIN: GET ALL FORMS (Active, Hidden, Closed)
// ==========================================
router.get('/forms/all', authenticateToken, requireAdmin, async (req, res) => {
    try {
        ...
        console.error('\n❌ [API ERROR] GET /forms/all (Fetch All Forms)');
        console.error('Error Details:', err.message);
...
// ==========================================
// 3. ADMIN: UPDATE FORM STATUS (Toggle Visibility)
// ==========================================
router.patch('/forms/:id', authenticateToken, requireAdmin, async (req, res) => {
    ...
    try {
        ...
        console.error(`\n❌ [API ERROR] PATCH /forms/${id} (Update Form)`);
        console.error('Payload Sent:', req.body);
        console.error('Error Details:', err.message);
...
router.get('/forms/:id/export', authenticateToken, requireAdmin, async (req, res) => {
    ...
    try {
        ...
        const fields = [
            ...
            'IFSC Code', 'Branch Name', 'Submitted At'
        ];
        ...
        console.error(`\n❌ [API ERROR] GET /forms/${id}/export (CSV Export)`);
        console.error('Error Details:', err.message);
...
router.get('/active', authenticateToken, async (req, res) => {
    try {
        ...
        console.error('\n❌ [API ERROR] GET /active (Student Fetch Active Forms)');
        console.error('Error Details:', err.message);
...
// ==========================================
// 6. STUDENT: GET OWN SUBMISSION (For pre-filling forms)
//    Also returns cleaned name + derived roll number for the frontend
// ==========================================
router.get('/forms/:form_id/my-submission', authenticateToken, async (req, res) => {
    ...
    try {
        // Fetch submission + user profile in one go
        const result = await db.query(`
            SELECT 
        ...
        // Also fetch user profile even if no submission yet
        ...
        res.json({
            // Always return cleaned profile info for form pre-filling
            profile: {
...
// ==========================================
// 7. STUDENT: SUBMIT / UPDATE REFUND DETAILS
// ==========================================
router.post('/forms/:form_id/submit', authenticateToken, async (req, res) => {
    ...
    // IFSC: 4 uppercase letters, literal '0', 6 alphanumeric chars
    ...
    try {
        ...
        console.error(`\n❌ [API ERROR] POST /forms/${form_id}/submit (Student Submission)`);
        console.error('Student ID:', student_id);
